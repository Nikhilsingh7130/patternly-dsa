...
async function getQuestions(db: D1Database, userId: number | null) { const result = await db.prepare("SELECT q.id, q.title, q.leetcodeNumber, q.section, q.pattern, q.difficulty, COALESCE(p.status, 'Not started') AS status, q.url, q.notes, q.source FROM questions q LEFT JOIN user_progress p ON p.questionId = q.id AND p.userId = ? WHERE q.source != 'Manual' OR q.source = 'Manual' AND q.createdBy = ? ORDER BY q.section, q.pattern, q.leetcodeNumber, q.id").bind(userId ?? -1, userId ?? -1).all<Question>(); return result.results ?? []; }
